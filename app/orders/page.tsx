        ...
        <div className="w-full px-6 lg:px-12 py-8 min-h-screen pt-24 bg-gradient-to-b from-[#070e09] via-[#064E3B] to-[#070e09] text-white">
            ...
            {orders.length === 0 ? (
                <div className="text-center bg-white/[0.03] p-8 rounded-2xl backdrop-blur-xl border border-white/[0.06]">
                    <p className="text-xl mb-6 text-white/50">You haven&apos;t placed any orders yet.</p>
                    <button
                        onClick={() => router.push('/')}
                        className="bg-[#ffd700] text-[#064E3B] px-6 py-3 rounded-xl font-bold hover:bg-[#ffde33] transition-all cursor-pointer"
                    >
                        Start Shopping
                    </button>
                </div>
                    ...
                    {orders.map((order) => (
                            ...
                            >
                                <div className="flex-1 min-w-0">
                                    <p className="text-[#ffd700]/80 font-mono text-base mb-1">#{order.id}</p>
                                    <p className="text-base text-white/40 mb-1">{order.line_items.length} items</p>
                                    {order.localDetails?.items?.slice(0, 3).map((item: any, idx: number) => (
                                        <div key={idx} className="flex items-center gap-2 flex-wrap">
                                            ...
                                            {item.sizeSelection && (
                                                <span className="text-sm bg-[#ffd700]/[0.06] text-[#ffd700]/60 px-1.5 py-0.5 rounded-full border border-[#ffd700]/[0.1]">
                                    ...
                                    {order.localDetails?.items?.length > 3 && (
                                        <span className="text-sm text-white/20">+{order.localDetails.items.length - 3} more</span>
                                    )}
                                    <p className="text-base capitalize mt-1 text-white/50">Status: <span className="font-semibold text-white/70">{order.status}</span></p>
                                </div>
